...
loadMatrixParser = do
  ...
  binwidth <- option auto ( long "binwidth" <> short 'b' <> metavar "BINSIZE" <> help "If input data has region features in the format of `chrN:START-END`, BINSIZE input is required to convert ranges to fixed width bins." )
  ...
  dropDimension <- switch $ option auto ( long "drop-dimension" <> help "Instead of keeping dimensions for --pca, --lsa, or --svd, drop the number of dimensions (i.e. --lsa 1 drops the first projection which may have a strong batch effect)." )
  filterThresholds <- optional $ option auto ( long "filter-thresholds" <> short 'H' <> metavar "(DOUBLE, DOUBLE)" <> help "The minimum filter thresholds for (MINCELL, MINFEATURE) when filtering cells and features by low read counts. Default changed to Nothing due to additional supported assays. To use the original filter thresholds, use --filter-thresholds \"(250, 1)\"." )
  shiftPositive <- switch ( long "shift-positive" <> metavar "Shift features to positive values. Positive values are shifted to allow modularity to work correctly." )
...
data MakeTree = MakeTree { loadMatrixOptions :: LoadMatrixOptions
               ...
               , drawCollection :: Maybe String
               , drawMark :: Maybe String
               , drawNodeNumber :: Bool
               , drawMaxNodeSize :: Maybe Double
               , drawMaxLeafNodeSize :: Maybe Double
               , drawNoScaleNodes :: Bool
               , drawLegendSep :: Maybe Double
               , drawLegendAllLabels :: Bool
               , drawPalette :: Maybe String
...
makeTreeParser = do
  ...
  minSize <- option auto ( long "min-size" <> short 'M' <> metavar "INT" <> value 1 <> showDefault <> help "The minimum size of a cluster. Defaults to 1." )
  ...
  drawNodeNumber <- switch ( long "draw-node-number" <> short 'N' <> metavar "Draw the node numbers on top of each node in the graph." )
  ...
  drawNoScaleNodes <- switch ( long "draw-no-scale-nodes" <> short 'W' <> metavar "Do not scale inner node size when drawing the graph. Instead, uses draw-max-node-size as the size of each node and is highly recommended to change as the default may be too large for this option." )
  ...
  drawLegendAllLabels <- switch ( long "draw-legend-all-labels" <> short 'J' <> metavar "Whether to show all the labels in the label file instead of only showing labels within the current tree. The program generates colors from all labels in the label file first in order to keep consistent colors. By default, this value is false, meaning that only the labels present in the tree are shown (even though the colors are the same). The subset process occurs after --draw-colors, so when using that argument make sure to account for all labels." )
  ...
  drawItemLineWeight <- option auto ( long "draw-item-line-weight" <> metavar "DOUBLE" <> value 0.1 <> showDefault <> help "The line weight for items in the leaves if shown. Supplied as if there are too many items, the collection may look like a black box. Set to 0 to disable outlines of items to avoid this." )
  drawFont <- option auto ( long "draw-font" <> metavar "FONT" <> value "Arial" <> showDefault <> help "Specify the font to use for the labels when plotting." )
  drawBarBounds <- switch ( long "draw-bar-bounds" <> metavar "Whether to plot only the minimum and maximum ticks for the color bars." )
  prior <- optional $ option auto ( long "prior" <> short 'p' <> metavar "STRING" <> help "The input folder containing the output from a previous run. If specified, skips clustering by using the previous clustering files." )
  noUpdateTreeRows <- switch ( long "no-update-tree-rows" <> metavar "Don't update the row indices of a tree if using an identical matrix to the one which generated the tree. Should not be used unless the matrix to make the tree is identical, then can result in speedup." )
  order <- option auto ( long "order" <> short 'O' <> metavar "DOUBLE" <> value 1 <> showDefault <> help "The order of diversity." )
  clumpinessMethod <- option auto ( long "clumpiness-method" <> short 'u' <> metavar "Majority | Exclusive | AllExclusive" <> value Majority <> showDefault <> help "The method used when calculating clumpiness: Majority labels leaves according to the most abundant label, Exclusive only looks at leaves consisting of cells solely from one label, and AllExclusive treats the leaf as containing both labels." )
  dense <- switch ( long "dense" <> metavar "Whether to use dense matrix algorithms for clustering. Should be faster for dense matrices, so if batch correction, PCA, or other algorithms are applied upstream to the input matrix, consider using this option to speed up the tree generation." )
  output <- option str ( long "output" <> short 'o' <> metavar "FOLDER" <> value "out" <> showDefault help "The folder containing output." )
...
interactiveParser = do
  loadMatrixOptions <-  loadMatrixParser
  labelsFile <- optional $ option auto ( long "labels-file" <> metavar "FILE" <> help "The input file containing the label for each cell barcode, with \"item,label\" header." )
...
data Differential = Differential { loadMatrixOptions :: LoadMatrixOptions
                   ...
                   , plotViolin :: Bool
                   , plotNoOutlier :: Bool
                   , plotOutput :: Maybe String
                   } deriving (Read, Show)

differentialParser :: Parser Subcommand
differentialParser
  loadMatrixOptions <-  loadMatrixParser
  labelsFile <- optional $ option str ( long "labels-file" <> metavar "FILE" <> help "The input file containing the label for each cell barcode, with \"item,label\" header." )
  prior <- optional $ option str ( long "prior" <> metavar "STRING" <> help "The input folder containing the output from a previous run. If specified, skips clustering by using the previous clustering files." )
  noUpdateTreeRows <- switch ( long "no-update-tree-rows" <> metavar "Don't update the row indices of a tree if using an identical matrix to the one which generated the tree. Should not be used unless the matrix to make the tree is identical, then can result in speedup." )
  edger <- switch ( long "edger" <> metavar "Use edgeR instead of Kruskall-Wallis for differential expression between two sets of nodes (automatically off and required to be off for all to all comparisons)." )
  ...
  features <- many $ option auto ( long "features" <> metavar "FEATURE" <> help "List of features (e.g. genes) to plot for all cells within selected nodes. Invoked by --features CD4 --features CD8 etc. When this argument is supplied, only the plot is outputted and differential expression is ignored. Outputs to --output." )
  ...
  plotOutput <- optional $ option str ( long "plot-output" <> metavar "FILE" <> value "out.pdf" <> showDefault <> help "The file containing the output plot." )
...
diversityParser =
  ...
  output <- option auto ( long "output" <> metavar "FOLDER" <> value "out" <> showDefault <> help "The folder containing output." )
...
data Paths = Paths { prior :: Maybe String
            ...
            , flipDirection :: Bool
            , shallowStart :: Bool
            , pathDistance :: Maybe String
            , bandwidth :: Maybe Double
            , delimiter :: Char
            , pathsPalette :: Maybe String
            , output :: Maybe String
            } deriving (Read, Show)
...
pathsParser = do
  ...
  output <- option str ( long "output" <> metavar "FOLDER" <> value "out" <> showDefault <> help "The folder containing output." )

  pure $ PathCommand (Paths {..})
...
data Peaks = Peaks { fragmentsPath :: [String]
            ...
            , peakCallCommand :: Maybe String
            , genomecovCommand :: Maybe String
            , genome :: Maybe String
            , skipFragments :: Bool
            , peakNode :: [Int]
...
peaksParser = do
  ...
  prior <- optional $ option str ( long "prior" <> metavar "STRING" <> help "The input folder containing the output from a previous run. If specified, skips clustering by using the previous clustering files." )
  delimiter <- delimiterParser
  labelsFile <- option str ( long "labels-file" <> metavar "FILE" <> help "The input file containing the label for each cell barcode, with \"item,label\" header." )
  ...
  peaksCustomRegion <- optional $ option str ( long "peaks-custom-region" <> metavar "chrN:START-END" <> help "Only look at these regions in the matrix for chromosome region features. A list in the format `chrN:START-END`, will assign values for a cell intersecting this region to this region. If a cell region overlaps two or more regions, will be counting that many times in the new features. Example input: `--custom-regions \"chr1:1003-10064\" --custom-regions \"chr2:3021-5034\"` etc." )
  ...
  skipFragments <- switch ( long "skip-fragments" <> metavar "Whether to skip the generation of the fragments (e.g. if changing only --peak-call-command and fragment separation by cluster already exists)." )
  ...
  allNodes <- switch ( long "allNodes" <> metavar "Whether to get fragments and peaks for all nodes, not just the leaves." )
  bedgraph <- switch ( long "bedgraph" <> metavar "Whether to output cluster normalized per million bedgraph output." )
  output <- option str ( long "output" <> metavar "FOLDER" <> value "out" <> showDefault <> help "The folder containing output."  )
...
motifsParser = do
  diffFile <- option auto ( long "diff-file" <> metavar "FILE" <> help "The input file containing the differential features between nodes. Must be in the format `node,feature,log2FC,pVal,qVal`. The node column is optional (if wanting to separate per node)." )
  ...
  motifGenome <- option auto ( long "motif-genome" <> metavar "FILE" <> help "The location of the genome file in fasta format to convert bed to fasta." )
  motifCommand <- optional $ option str ( long "motif-command" <> metavar "STRING" <> value "meme %s -nmotifs 50 -oc %s" <> showDefault <> help "The command to find motifs in a fasta file. Can be any command that will be run on each fasta file converted from the bed optionally per node, but the first \"%s\" must be the input file, the second \"%s\" is the output. An example of homer: `/path/to/findMotifs.pl %s fasta %s`. Uses meme by default." )
...
--                , binarize :: Bool <?> "Binarize data. Default for fragments.tsv.gz."
...
--                , blacklistRegionsFile :: Maybe T.Text <?> "([Nothing] | FILE) Bed file containing regions to ignore. Any fragments overlapping these regions are ignored if the input is not a fragments file."
--                , customRegion :: [T.Text] <?> "([Nothing] | chrN:START-END) Only look at these regions in the matrix for chromosome region features. A list in the format `chrN:START-END`, will assign values for a cell intersecting this region to this region. If a cell region overlaps two or more regions, will be counting that many times in the new features. Example input: `--custom-regions \"chr1:1003-10064\" --custom-regions \"chr2:3021-5034\"` etc."
...
--                , normalization :: [String] <?> "([TfIdfNorm] | UQNorm | MedNorm | TotalMedNorm | TotalNorm | LogCPMNorm DOUBLE | QuantileNorm | NoneNorm) Type of normalization before clustering. Can be used as a list to perform one after the other: --normalization QuantileNorm --normalization TfIdfNorm will first apply quantile then tf-idf normalization. TfIdfNorm normalizes based on the prevalence of each feature. UQNorm normalizes each observation by the upper quartile non-zero counts of that observation. MedNorm normalizes each observation by the median non-zero counts of that observation. TotalNorm normalizes each cell by the total count. TotalMedNorm normalized first each observation by total count then by median of non-zero counts across features. (LogCPMNorm DOUBLE) normalizes by logB(CPM + 1) where B is DOUBLE. QuantileNorm normalizes by quantile normalization, ignores zeros, may be slow. NoneNorm does not normalize. Default is TfIdfNorm for clustering and NoneNorm for differential (which instead uses the recommended edgeR single cell preprocessing including normalization and filtering if requested, any normalization provided here will result in edgeR preprocessing on top). Older versions had BothNorm which has been replaced with --normalization TotalMedNorm --normalization TfIdfNorm. This change will also affect other analyses, as TfIdfNorm will now be in non-cluster-related entry points if specified, so --normalization UQNorm from < v2.0.0.0 is now --normalization UQNorm --normalization TfIdfNorm."
...
--                , matrixOutput :: Maybe String <?> "([Nothing] | FOLDER | FILE.csv) Output the filtered and normalized (not including TfIdfNorm) matrix in this folder under the --output directory in matrix market format or, if a csv file is specified, a dense csv format. Like input, features are rows."
--                , labelsOutput :: Bool <?> "Whether to write the labels used for each observation as a labels.csv file in the output folder."
--                , fragmentsOutput :: Bool <?> "Whether to output fragments_tsv.gz with barcodes altered by --custom-label in the output folder (excludes filtered-out cells). Useful for downstream analysis by the peaks entry point where the cluster barcodes differ from the original fragments.tsv.gz file when using --custom-label. Matches barcodes based on BARCODE-LABEL."
...
--                , dropDimension :: Bool <?> "Instead of keeping dimensions for --pca, --lsa, or --svd, drop the number of dimensions (i.e. --lsa 1 drops the first projection which may have a strong batch effect)."
--                , shiftPositive :: Bool <?> "Shift features to positive values. Positive values are shifted to allow modularity to work correctly."
--                , filterThresholds :: Maybe String <?> "([Nothing] | (DOUBLE, DOUBLE)) The minimum filter thresholds for (MINCELL, MINFEATURE) when filtering cells and features by low read counts. Default changed to Nothing due to additional supported assays. To use the original filter thresholds, use --filter-thresholds \"(250, 1)\"."
...
--                , noUpdateTreeRows :: Bool <?> "Don't update the row indices of a tree if using an identical matrix to the one which generated the tree. Should not be used unless the matrix to make the tree is identical, then can result in speedup."
...
--                    , prior :: Maybe String <?> "([Nothing] | STRING) The input folder containing the output from a previous run. If specified, skips clustering by using the previous clustering files."
--                    , noUpdateTreeRows :: Bool <?> "Don't update the row indices of a tree if using an identical matrix to the one which generated the tree. Should not be used unless the matrix to make the tree is identical, then can result in speedup."
--                    , edger :: Bool <?> "Use edgeR instead of Kruskall-Wallis for differential expression between two sets of nodes (automatically off and required to be off for all to all comparisons)."
...
--                    , features :: [T.Text] <?> "([Nothing] | FEATURE) List of features (e.g. genes) to plot for all cells within selected nodes. Invoked by --features CD4 --features CD8 etc. When this argument is supplied, only the plot is outputted and differential expression is ignored. Outputs to --output."
...
--                    , plotSeparateLabels :: Bool <?> "([False] | True) Whether to plot each label separately. This will plot each label provided in --labels from both entries in the tuple (as they may be different from --nodes)."
--                    , plotViolin :: Bool <?> "([False] | True) Whether to plot features as a violin plots instead of boxplots."
--                    , plotNoOutlier :: Bool <?> "([False] | True) Whether to avoid plotting outliers as there can be too many, making the plot too large."
...
--             , output :: Maybe String <?> "([out] | STRING) The folder containing output." }
--     | Motifs { diffFile :: T.Text <?> "(FILE) The input file containing the differential features between nodes. Must be in the format `node,feature,log2FC,pVal,qVal`. The node column is optional (if wanting to separate per node)."
...
--                    , filterThresholds :: Maybe String <?> "([Nothing] | (DOUBLE, DOUBLE)) The minimum filter thresholds for (MINCELL, MINFEATURE) when filtering cells and features by low read counts. Default changed to Nothing due to additional supported assays. To use the original filter thresholds, use --filter-thresholds \"(250, 1)\"."
--                    , shiftPositive :: Bool <?> "Shift features to positive values. Positive values are shifted to allow modularity to work correctly."
--                    , matOutput :: String <?> "([out_matrix] | FOLDER | FILE.csv) Output the filtered and normalized (not including TfIdfNorm) matrix in this folder in matrix market format or, if a csv file is specified, a dense csv format. Like input, features are rows." }
--     deriving (Generic)

-- modifiers :: Modifiers
-- modifiers = lispCaseModifiers { shortNameModifier = short }
--   where
--     short "allNodes"              = Nothing
--     short "aggregate"             = Nothing
--     short "atac"                  = Nothing
--     short "binarize"              = Nothing
--     short "bedgraph"              = Nothing
--     short "blacklistRegionsFile"  = Nothing
--     short "customRegion"          = Nothing
...
--     short "fragmentsPath"         = Just 'f'
--     short "genome"                = Nothing
--     short "genomecovCommand"      = Nothing
--     short "labels"                = Nothing
--     short "labelsOutput"          = Nothing
--     short "lsa"                   = Nothing
--     short "matOutput"             = Nothing
--     short "matrixOutput"          = Nothing
--     short "matrixTranspose"       = Just 'T'
...
--     short "plotSeparateLabels"    = Nothing
