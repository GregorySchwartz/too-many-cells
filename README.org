...
#+BEGIN_SRC sh
sudo docker run -it --rm -v "/home/username:/home/username" gregoryschwartz/too-many-cells:0.1.5.0 -h
#+END_SRC
...
above example to handle both input and output. Specifically, =-v
/path/to/matrix/on/host:/input_matrix= with =-m /input_matrix= is what you want,
where before the =:= is on the host filesystem while after the =:= is what the
docker program sees. Then you can write the output in the same way: =-v
/path/to/output/on/host:/output= will write the output to the folder before the
=:=.
