...
different perspective of single cells, using our [[http://github.com/GregorySchwartz/birch-beer#readme][=birch-beer=]] visualization and
tree measures to describe simultaneously large and small populations, without
...
=stack= [[Install =stack= ][see below]]), but we also have a =Dockerfile= to use in any system in case
you have a custom build (for instance, a non-standard R installation) or
...
just need to follow the suggestion and add the dependencies to the specified file.
...
single cell matrices: =--matrix-path input1 --matrix-path input2=.
