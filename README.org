...
- Support for scATAC-seq for chromatin state relationships.
...
- Now packaged for the functional package manager =nix=! No more dependency
...
with [[nix][ =nix= ]] or [[Stack][ =stack= ]]. =nix= will provide all dependencies in the build,
supports linux and macOS, and should be reproducible, so try that first. We also
have [[Docker][docker]] images and a =Dockerfile= to use in any system in case you have a
custom build (for instance, a non-standard R installation) or difficulty
installing. *macOS and Windows users:* =too-many-cells= was built and tested on
linux, so we highly recommend using the =nix= (macOS) or [[Docker][docker]] (macOS or
Windows) image (which a completely isolated environment which requires no
...
We recommend using =nix= or [[Docker][docker]] on macOS. If you need to build
...
To get =too-many-cells= (replace 0.2.2.0 with [[https://cloud.docker.com/repository/docker/gregoryschwartz/too-many-cells/general][any version needed]]):
...
#+BEGIN_SRC sh
sudo docker run -it --rm -v "/home/username:/home/username" gregoryschwartz/too-many-cells:0.2.2.0 -h
#+END_SRC
...
3. Remove features with less than 1 count (=--filter-thresholds=, =--no-filter=).
...
be sure to use =--no-filter --normalization NoneNorm= (and =--shift-positive=
for PCA) to avoid wrong filters and scalings! *If using dimensionality reduction
...
relationships from chromatin state information. All of the previous analyses
used with gene-product features now work with genomic regions in the form
=chrN:START-END=, where =N= is the chromosome number, =START= is the start of
...
we try to overcome a potential issue where all cells are considered outliers. We
also change the =--filter-thresholds= as the default thresholds are for
scRNA-seq. To blacklist known biased regions in the genome, we can call
...
and =labels.csv= was from the [[Working with scATAC-seq data][previous scATAC-seq section]], where we
