...
To get =too-many-cells= (replace 0.1.1.0 with [[https://cloud.docker.com/repository/docker/gregoryschwartz/too-many-cells/general][any version needed]]):

#+BEGIN_SRC sh
docker pull gregoryschwartz/too-many-cells:0.1.1.0
#+END_SRC

To run =too-many-cells= in a docker container:

#+BEGIN_SRC sh
sudo docker run gregoryschwartz/too-many-cells:0.1.1.0 -h
#+END_SRC
...
above) or a file (a =csv= file). *You don't need to use scRNA-seq data!* You can
...
to use =--no-filter --normalization NoneNorm= to avoid wrong filters and
