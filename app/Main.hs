...
data Options
    = MakeTree { matrixPath :: [String] <?> "(PATH) The path to the input directory containing the matrix output of cellranger (matrix.mtx, genes.tsv, and barcodes.tsv) or, if genes-file and cells-file are not specified, or an input csv file containing gene row names and cell column names. If given as a list (--matrixPath input1 --matrixPath input2 etc.) then will join all matrices together. Assumes the same number and order of genes in each matrix, so only cells are added."
               ...
               , normalization :: Maybe String <?> "([B1Norm] | UQNorm | MaxMedNorm | BothNorm | NoneNorm) Type of normalization before clustering. B1Norm normalizes based on the prevalence of each feature. UQNorm normalized each observation by the upper quartile non-zero counts of that observation. MaxMedNorm normalized first each observation by total count then by median across features. BothNorm uses both normalizations (first UQNorm for all analysis then additionally B1Norm during clustering). NoneNorm does not normalize. Default is B1Norm for clustering and None for differential (edgeR). Cannot use B1Norm for any other process as NoneNorm will become the default."
...
loadAllSSM opts = do
    ...
    let whiteListFilter Nothing = id
        ...
        normMat BothNorm     = uqScaleSparseMat
        normMat NoneNorm     = id
